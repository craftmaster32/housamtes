...
describe('splitMoney', (): void => {
  it('splits a shekel amount into aligned symbol, whole, and fraction', (): void => {
    expect(splitMoney(746.46, 'ILS')).toEqual({ symbol: '₪', whole: '746', fraction: '.46' });
  });

  it('keeps thousands separators in the whole part', (): void => {
  ...
  it('returns an empty fraction for zero-decimal currencies (JPY)', (): void => {
  ...
  it('recomposes to the same string formatFull produces', (): void => {
    const { symbol, whole, fraction } = splitMoney(46, 'ILS');
    expect(`${symbol}${whole}${fraction}`).toBe(formatFull(46, 'ILS'));
  });

  it('falls back gracefully for an unknown currency code', (): void => {
