...
// Timezone for all date/time comparisons — must match the timezone in which
// reservation start_time strings are entered (they are stored as plain "HH:MM"
// local-time text, not UTC).  Set HOUSE_TIMEZONE in Supabase function secrets.
const HOUSE_TIMEZONE = Deno.env.get('HOUSE_TIMEZONE') ?? 'UTC';

// Returns the current time expressed in HOUSE_TIMEZONE so we can compare
// against local-time start_time strings stored in the DB.
function getLocalContext(): { nowMins: number; today: string; tomorrow: string } {
  const now = new Date();
...
Deno.serve(async (_req: Request): Promise<Response> => {
  ...
  try {
    const { nowMins, today, tomorrow } = getLocalContext();
    ...
    for (const r of allReservations) {
      ...
      // r.date === tomorrow already guarantees we're within ~24-48h; no further
      // time math needed (and avoids UTC-vs-local issues with bare "HH:MM" strings).
