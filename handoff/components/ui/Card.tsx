...
// - Optional `entering` prop accepts any Reanimated entering animation
//   (FadeIn, FadeInDown, etc.) so callers can stagger reveals.
...
interface Props {
  ...
  // Reanimated entering / layout — pass through so callers can stagger.
  // Typed loosely to avoid forcing every caller to import reanimated.
...
export function Card({
  ...
  if (onPress) {
    return (
      <Animated.View
        // @ts-expect-error reanimated props typed via JSX
        entering={entering}
        layout={layout}
        style={animatedStyle}
      >
  ...
  // Non-pressable path — still supports `entering` for staggered reveals.
  if (entering || layout) {
    return (
      <Animated.View
        // @ts-expect-error reanimated props typed via JSX
